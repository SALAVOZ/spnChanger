...
from kerberosv5_fake import getKerberosTGS
...
class GETST:
    ...
    def saveTicket(self, ticket, sessionKey, doYouChangeTicket=False):
        logging.info('Saving ticket in %s' % (self.__saveFileName + '.ccache'))
        ccache = CCache()

        ccache.fromTGS(ticket, sessionKey, sessionKey, doYouChangeTicket)
    ...
    def run(self):
        ...
        print('[*] Writing key...')
        open('key', 'wb').write(oldSessionKey.contents)
        self.saveTicket(tgs, oldSessionKey, doYouChangeTicket=False)
...
if __name__ == '__main__':
    ...
    group.add_argument('-krb5ccname', action='store', metavar="krb5-file", help='path to tgt file')
    ...
    os.environ['KRB5CCNAME'] = f'{options.krb5ccname}';
