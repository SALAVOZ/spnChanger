...
class Credential:
    ...
    def toTGS(self, newSPN=None):
        ...
        b_salavata = open('encrypted_part', 'rb').read()
        c_salavata: Any = OctetString(b_salavata)
        tgs_rep['enc-part']['etype'] = 18
        tgs_rep['enc-part']['cipher'] = str(c_salavata)
        #
...
class CCache:
    ...
    def fromTGS(self, tgs, oldSessionKey, sessionKey, doYouChangeTicket):
        self.headers = []
        header = Header()
        header['tag'] = 1
        header['taglen'] = 8
        ...
        #MY CODE
        if doYouChangeTicket:
            server.components[0] = 'CIFS'
        #
        tmpServer = Principal()
        tmpServer.fromPrincipal(server)
        credential['client'] = self.principal
